Last synced commit: b5fb1d1bce0f101dd63efaaf32d17a2d528bb71d (2026-08-03, branch main)
...
- Project 10's work (Phases 1–6 of the roadmap) was merged to `main` via PR #20 (`c7a4b41`) on 2026-07-31, then the `PROJECT_STATE.md` sync commit (`fd143cf`) via PR #21 (`af89b99`). Since then, work has continued **directly on `main`** (no active feature branch) — the cookie/CSRF auth layer (below) landed as 2 commits (`5623b23`, `b5fb1d1`) straight on `main`, not through a PR. Confirm with `git log --oneline origin/main..HEAD` before assuming a future session's local `main` matches this if picking up elsewhere.
...
**Auth transport (2026-08-03 — reversed from earlier in the project):** originally Bearer-header-only (deliberately, to differ from project 09). User decided project 10 will get its own web frontend like project 09's, so cookie support was added **on top of**, not instead of, Bearer — `AuthController` now sets `access_token`/`refresh_token` httpOnly cookies via `CookieUtils` (filled in this round, mirrors project 09's implementation) *and* still returns the same tokens in the JSON body (kept for Postman/mobile/Bearer clients). `JwtAuthenticationFilter.resolveToken()` checks the cookie first, falls back to the header. CSRF was re-enabled (`CookieCsrfTokenRepository` + new `CsrfCookieFilter`, both mirroring project 09) but scoped so it **only applies to non-Bearer requests** (`SecurityConfig.CSRF_REQUIRED_MATCHER`) — otherwise every Postman/mobile mutating call would 403. `CorsConfig` changed from wildcard-origin+no-credentials to a specific `app.frontend-url` origin + `allowCredentials(true)` (required for cross-origin cookies to work at all).
- **Two bugs found and fixed during self-review before this was called done:** the CSRF ignore list initially used `/api/v1/auth/**` wildcard — same class of trap as the plan's intentionally-seeded Step 6.8 bug, except introduced fresh this time (would've exempted `DELETE /api/v1/auth/sessions/{id}` from CSRF); and CSRF would've broken all Bearer-token clients before the `CSRF_REQUIRED_MATCHER` fix was added.
- **Known, accepted trade-off (not a bug, documented in the plan doc §8):** returning tokens in the JSON body *and* as httpOnly cookies means an XSS payload on the eventual web frontend could still read the token via the fetch/axios response even though it can't read the cookie directly — unavoidable while the same endpoints serve both cookie and Bearer clients. No frontend exists yet to actually exploit or verify this against; noted for whenever one is built.
- Full rationale for all of the above: `docs/plans/10-spring-boot-blueprint.md` §8 (decisions table, entries dated 2026-08-01) and §9 (checklist) — not duplicated here.

**Still not done:** no Flyway migrations (`ddl-auto: update` only), no `application-dev.yml`/`application-prod.yml` split (so `CookieUtils`' `secure(false)` is still hardcoded dev-only — commented as such, needs flipping to `true` whenever that split happens), no tests beyond the `ApplicationTests` `contextLoads()` stub (Phase 7, next up), no actual frontend project yet despite the cookie support being added in anticipation of one. `OpenApiConfig`/`LoggingAspect`/`DateUtils` still empty stubs, intentionally deferred to Phase 8.
